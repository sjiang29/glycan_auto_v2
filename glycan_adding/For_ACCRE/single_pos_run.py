...
'''
    ...
    Args:
        This needs to be 3 input from user
        usage: 'python single_pos_run.py < aa index> <n_struct> <name of csv file that list the interested antigesn>' in the terminal, /
...
def main(argv):
        if len(argv) != 3:
            ...
            glycan_pos = int(sys.argv[1])
            #end_pos = int(sys.argv[2])
            n_struct = int(sys.argv[2])
            antigens_csv = sys.argv[3]
